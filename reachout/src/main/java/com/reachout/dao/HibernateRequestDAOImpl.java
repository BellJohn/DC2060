...
public class HibernateRequestDAOImpl extends HibernateListingDAOImpl {
	...
	public HashMap<Integer, String> getCreatedBy(int userId) {
		...
		try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/reach_out", "reach", "reach_pass");
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT USERS_ID, USERS_USERNAME FROM USERS u JOIN LISTINGS l ON u.USERS_ID = l.LST_USER_ID where l.LST_TYPE = " + ListingType.REQUEST.getOrdindal() + " AND u.USERS_ID != " + userId + " AND l.LST_STATUS = 0")) {
			while (rs.next()) {
