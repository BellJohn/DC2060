				...
				<c:choose>
					<c:when test="${conversations.size() > 0}">
						<div class="row">
							...
							<div class="col-lg-9">
								<div class="card"
									style="display: inline-flex; float: inline-end; visibility: hidden; width: 100%;">
									<div id="sendMSGBox" class="input-group"
										style="visibility: hidden">
										<input type="text" class="form-control"
											placeholder="Send Message" aria-label="Send Message"
											aria-describedby="button-sendMSG" id="inputMessage" />
							...
							complete : function(resp) {
								...
								for (var i = 0; i < obj.length; i++) {
									...
									if (activeUID == obj[i].userOther) {
										content += "<button id=\"tablink_"+obj[i].userOther+"\" class=\"tablinks active\"  onclick=\"openConvo(event, "
												+ obj[i].userOther
												+ ")\">"
												+ obj[i].otherUserName
												+ "</button>";
										...
										content += "<button id=\"tablink_"+obj[i].userOther+"\" class=\"tablinks\"  onclick=\"openConvo(event, "
												+ obj[i].userOther
												+ ")\">"
												+ obj[i].otherUserName
												+ "</button>";
